...
function note_report( $pid, $encounter, $cols, $id) {
    ...
    if ($data) {
        ...
        foreach($data as $key => $value) {
            ...
            print("<tr>\n");
	    if ($key == "Note Type") {
                print "<td style='width:50%'><span class=bold>" . xlt($key) . ": </span><span class=text>" . xlt($value) . "</span></td>";
	    }
	    else {
	        print "<td style='width:50%'><span class=bold>" . xlt($key) . ": </span><span class=text>" . text($value) . "</span></td>";	
	    }
            $count++;
            if ($count == $cols) {
                $count = 0;
